...
class CollectdReporter::Impl {
 public:
  Impl(CollectdReporter& self, MetricsRegistry& registry,
       const std::string& hostname = "127.0.0.1", std::uint16_t port = 25826);
  ~Impl();
  void Run();
...
CollectdReporter::CollectdReporter(MetricsRegistry& registry,
...
void CollectdReporter::Run() { impl_->Run(); }

void CollectdReporter::Process(Counter& counter) { impl_->Process(counter); }

void CollectdReporter::Process(Meter& meter) { impl_->Process(meter); }

void CollectdReporter::Process(Histogram& histogram) { impl_->Process(histogram); }

void CollectdReporter::Process(Timer& timer) { impl_->Process(timer); }

void CollectdReporter::Process(Value& value) { impl_->Process(value); }

// === Implementation ===

CollectdReporter::Impl::Impl(CollectdReporter& self, MetricsRegistry& registry,
                             const std::string& hostname, std::uint16_t port)
    : self_(self), registry_(registry) {
...
void CollectdReporter::Impl::Run() {
  ...
  for (auto& kv : registry_.GetAllMetrics()) {
    ...
    //                std::cout << msgbuf_ << std::endl;
    //                std::cout << msgbuf_ptr_ << std::endl;
    ...
    //                }
    //                std::cout << std::endl;
    int num = sendto(socket_, msgbuf_, msg_size, 0, addrinfo_->ai_addr,
...
void CollectdReporter::Impl::Process(Value& value) {
  AddPart(kType, "cpp_metrics_value");
  AddPart(kTypeInstance, current_instance_);
  AddValues({{kGauge, static_cast<double>(value.value())}});
}

void CollectdReporter::Impl::Process(Timer& timer) {
  auto snapshot = timer.GetSnapshot();
  AddPart(kType, "medida_timer");
  AddPart(kTypeInstance,
          current_instance_ + "." + FormatRateUnit(timer.duration_unit()));
  AddValues({
      {kGauge, timer.min()},
      {kGauge, timer.max()},
      {kGauge, timer.mean()},
...
void CollectdReporter::Impl::AddValues(
    std::initializer_list<TypeValue> values) {
...
void CollectdReporter::Impl::pack_double(double data) {
  // FIXME: Should break on bigendian archs. collectd expects little-endian
  // doubles
